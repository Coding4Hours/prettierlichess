...
chrome.storage.sync.get("toggledCustomBoard", (result) => {
				...
				if (tabs[i].url !== undefined) {
					...
					if (chrome.scripting?.executeScript) {
						...
						const invocation =
							...
							");";
						chrome.tabs.executeScript(tabs[i].id, { code: invocation }, () => {
...
function importScheme(data) {
	chrome.storage.sync.get(null, (result) => {
		...
		for (const scheme in defaultColorScheme) {
			if (json[scheme]) {
...
document.querySelector("#layoutSelect").addEventListener("change", () => {
...
chrome.storage.sync.get("defaultBoardSwitch", (result) => {
	if (result.defaultBoardSwitch) {
